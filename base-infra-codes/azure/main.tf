...
resource "azurerm_virtual_network" "app-vnet" {
  count               = 2
  name                = "pod${var.pod_number}-app${count.index + 1}-vnet-azure"
  location            = var.location
...
resource "azurerm_subnet" "app-subnet" {
  count                = 2
  name                 = "pod${var.pod_number}-app${count.index + 1}-subnet-azure"
  resource_group_name  = azurerm_resource_group.app-rg["${count.index}"].name
...
resource "azurerm_route" "jumpbox1_route" {
  ...
  address_prefix      = "68.154.48.186/32"
...
locals {
  vn_cidr1     = ["10.2.${var.pod_number}.0/24"]
  vn_cidr2     = ["10.2.${var.pod_number + 100}.0/24"]
  subnet_cidr1 = ["10.2.${var.pod_number}.0/26"]
  subnet_cidr2 = ["10.2.${var.pod_number + 100}.0/26"]
  app1_nic     = "10.2.${var.pod_number}.10"
  app2_nic     = "10.2.${var.pod_number + 100}.10"
}
